...
#define RA8E1_I2C0_BASE         0x40072000
#define RA8E1_I2C_SPCR_OFFSET   0x00    /* SPI Control Register */
#define RA8E1_I2C_SSLP_OFFSET   0x01    /* SPI Slave Select Polarity Register */
#define RA8E1_I2C_SPPCR_OFFSET  0x02    /* SPI Pin Control Register */
#define RA8E1_I2C_SPSR_OFFSET   0x03    /* SPI Status Register */
#define RA8E1_I2C_SPDR_OFFSET   0x04    /* SPI Data Register */
#define RA8E1_I2C_SPSCR_OFFSET  0x08    /* SPI Sequence Control Register */
#define RA8E1_I2C_SPSSR_OFFSET  0x09    /* SPI Sequence Status Register */
#define RA8E1_I2C_SPBR_OFFSET   0x0A    /* SPI Bit Rate Register */
#define RA8E1_I2C_SPDCR_OFFSET  0x0B    /* SPI Data Control Register */
#define RA8E1_I2C_SPCKD_OFFSET  0x0C    /* SPI Clock Delay Register */
#define RA8E1_I2C_SSLND_OFFSET  0x0D    /* SPI Slave Select Negation Delay Register */
#define RA8E1_I2C_SPND_OFFSET   0x0E    /* SPI Next-Access Delay Register */
#define RA8E1_I2C_SPCR2_OFFSET  0x0F    /* SPI Control Register 2 */
#define RA8E1_I2C_SPCMD0_OFFSET 0x10    /* SPI Command Register 0 */

/* SPI Control Register (SPCR) bits */
#define RA8E1_I2C_SPCR_SPRIE    (1 << 7)  /* SPI Receive Interrupt Enable */
#define RA8E1_I2C_SPCR_SPE      (1 << 6)  /* SPI Function Enable */
#define RA8E1_I2C_SPCR_SPTIE    (1 << 5)  /* SPI Transmit Interrupt Enable */
#define RA8E1_I2C_SPCR_SPEIE    (1 << 4)  /* SPI Error Interrupt Enable */
#define RA8E1_I2C_SPCR_MSTR     (1 << 3)  /* SPI Master/Slave Mode Select */
#define RA8E1_I2C_SPCR_MODFEN   (1 << 2)  /* Mode Fault Error Detection Enable */
#define RA8E1_I2C_SPCR_TXMD     (1 << 1)  /* Communications Operating Mode Select */
#define RA8E1_I2C_SPCR_SPMS     (1 << 0)  /* SPI Mode Select */

/* SPI Status Register (SPSR) bits */
#define RA8E1_I2C_SPSR_SPRF     (1 << 7)  /* SPI Receive Buffer Full Flag */
#define RA8E1_I2C_SPSR_SPTEF    (1 << 5)  /* SPI Transmit Buffer Empty Flag */
#define RA8E1_I2C_SPSR_PERF     (1 << 3)  /* Parity Error Flag */
#define RA8E1_I2C_SPSR_MODF     (1 << 2)  /* Mode Fault Error Flag */
#define RA8E1_I2C_SPSR_IDLNF    (1 << 1)  /* SPI Idle Flag */
#define RA8E1_I2C_SPSR_OVRF     (1 << 0)  /* Overrun Error Flag */

/* SPI Command Register (SPCMD0) bits */
#define RA8E1_I2C_SPCMD_SCKDEN  (1 << 15) /* RSPCK Delay Setting Enable */
#define RA8E1_I2C_SPCMD_SLNDEN  (1 << 14) /* SSL Negation Delay Setting Enable */
#define RA8E1_I2C_SPCMD_SPNDEN  (1 << 13) /* SPI Next-Access Delay Enable */
#define RA8E1_I2C_SPCMD_LSBF    (1 << 12) /* SPI LSB First */
#define RA8E1_I2C_SPCMD_SPB_MASK (0x0F << 8) /* SPI Data Length Setting */
#define RA8E1_I2C_SPCMD_SPB_8   (0x07 << 8)  /* 8 bits */
#define RA8E1_I2C_SPCMD_SPB_16  (0x0F << 8)  /* 16 bits */
#define RA8E1_I2C_SPCMD_SSLKP   (1 << 7)  /* SSL Signal Level Keeping */
#define RA8E1_I2C_SPCMD_SSLA_MASK (0x07 << 4) /* SSL Signal Assertion Setting */
#define RA8E1_I2C_SPCMD_BRDV_MASK (0x03 << 2) /* Bit Rate Division Setting */
#define RA8E1_I2C_SPCMD_CPOL    (1 << 1)  /* RSPCK Polarity Setting */
#define RA8E1_I2C_SPCMD_CPHA    (1 << 0)  /* RSPCK Phase Setting */
...
#define RA8E1_MSTP_I2C0         (1 << 22)    /* SPI0 Module Stop */
#define RA8E1_MSTP_DTC          (1 << 28)    /* DTC Module Stop */

/* IRQ Numbers */
#define RA8E1_IRQ_I2C0_RXI      142
#define RA8E1_IRQ_I2C0_TXI      143
#define RA8E1_IRQ_I2C0_ERI      144
...
static uint32_t spi_gy912_setfrequency(struct spi_dev_s *dev, uint32_t frequency)
{
  ...
  /* Configure bit rate */
  spi_gy912_putreg8(priv, RA8E1_I2C_SPBR_OFFSET, spbr);

  /* Update command register with BRDV */
  uint16_t spcmd = spi_gy912_getreg16(priv, RA8E1_I2C_SPCMD0_OFFSET);
  spcmd &= ~RA8E1_I2C_SPCMD_BRDV_MASK;
  spcmd |= (brdv << 2) & RA8E1_I2C_SPCMD_BRDV_MASK;
  spi_gy912_putreg16(priv, RA8E1_I2C_SPCMD0_OFFSET, spcmd);
  ...
  if (mode != priv->mode)
    {
      ...
      spcmd = spi_gy912_getreg16(priv, RA8E1_I2C_SPCMD0_OFFSET);
      spcmd &= ~(RA8E1_I2C_SPCMD_CPOL | RA8E1_I2C_SPCMD_CPHA);
      ...
      switch (mode)
        {
          ...
          case SPIDEV_MODE1: /* CPOL=0; CPHA=1 */
            spcmd |= RA8E1_I2C_SPCMD_CPHA;
            break;

          case SPIDEV_MODE2: /* CPOL=1; CPHA=0 */
            spcmd |= RA8E1_I2C_SPCMD_CPOL;
            break;
  ...
  if (nbits != priv->nbits)
    {
      ...
      spcmd = spi_gy912_getreg16(priv, RA8E1_I2C_SPCMD0_OFFSET);
      spcmd &= ~RA8E1_I2C_SPCMD_SPB_MASK;

      if (nbits <= 8)
        {
          spcmd |= RA8E1_I2C_SPCMD_SPB_8;
          priv->nbits = 8;
        }
      else if (nbits <= 16)
        {
          spcmd |= RA8E1_I2C_SPCMD_SPB_16;
          priv->nbits = 16;
        }
      else
        {
...
static uint32_t spi_gy912_send(struct spi_dev_s *dev, uint32_t wd)
{
  ...
  if (priv->nbits <= 8)
    {
      spi_gy912_putreg8(priv, RA8E1_I2C_SPDR_OFFSET, (uint8_t)wd);
    }
  else
    {
      spi_gy912_putreg16(priv, RA8E1_I2C_SPDR_OFFSET, (uint16_t)wd);
    }

  /* Wait until the transfer is complete */
  do
    {
      regval = spi_gy912_getreg8(priv, RA8E1_I2C_SPSR_OFFSET);
    }
  while ((regval & RA8E1_I2C_SPSR_SPTEF) == 0);
  ...
  while ((regval & RA8E1_I2C_SPSR_SPRF) == 0);

  /* Read the received data */
  if (priv->nbits <= 8)
    {
      ret = spi_gy912_getreg8(priv, RA8E1_I2C_SPDR_OFFSET);
    }
  else
    {
      ret = spi_gy912_getreg16(priv, RA8E1_I2C_SPDR_OFFSET);
    }
                               ...
                               void *rxbuffer, size_t nwords)
{
  ...
  xfer.status = RA8E1_I2C_STATUS_IDLE;
...
static void spi_gy912_bus_initialize(struct spi_gy912dev_s *priv)
{
  ...
  ra_configgpio(GPIO_I2C0_SCK);
  ra_configgpio(GPIO_I2C0_MOSI);
  ra_configgpio(GPIO_I2C0_MISO);
  ra_configgpio(GPIO_I2C0_SS0);
  ra_configgpio(GPIO_I2C0_BMP_CS);

  /* Enable SPI0 clock */
  modifyreg32(RA8_SYSTEM_MSTPCRB, RA8E1_MSTP_I2C0, 0);

  /* Disable SPI function */
  spi_gy912_putreg8(priv, RA8E1_I2C_SPCR_OFFSET, 0);

  /* Configure SPI Control Register */
  regval = RA8E1_I2C_SPCR_MSTR |    /* Master mode */
           RA8E1_I2C_SPCR_SPE;      /* SPI function enable */
  spi_gy912_putreg8(priv, RA8E1_I2C_SPCR_OFFSET, regval);

  /* Configure SPI Command Register 0 */
  spcmd = RA8E1_I2C_SPCMD_SPB_8 |   /* 8-bit data length */
          (0 << 4);                 /* SSL0 assertion */
  spi_gy912_putreg16(priv, RA8E1_I2C_SPCMD0_OFFSET, spcmd);
  ...
  ra_gpiowrite(GPIO_I2C0_SS0, true);
  ra_gpiowrite(GPIO_I2C0_BMP_CS, true);
                               ...
                               struct spi_gy912_transfer_s *xfer)
{
  ...
  xfer->status = RA8E1_I2C_STATUS_BUSY;
  ...
  xfer->status = RA8E1_I2C_STATUS_COMPLETE;
  xfer->result = OK;
  ...
  switch (devid)
    {
      case SPIDEV_IMU(0):
        ...
        ra_gpiowrite(GPIO_I2C0_SS0, !select);
        break;

      case SPIDEV_BAROMETER(0):
        /* Barometer (BMP388) on GPIO CS */
        ra_gpiowrite(GPIO_I2C0_BMP_CS, !select);
        break;
...
int spi_gy912_interrupt_handler(int irq, void *context, void *arg)
{
  ...
  status = spi_gy912_getreg8(priv, RA8E1_I2C_SPSR_OFFSET);

  /* Handle errors */
  if (status & (RA8E1_I2C_SPSR_OVRF | RA8E1_I2C_SPSR_MODF | RA8E1_I2C_SPSR_PERF))
    {
      spierr("SPI Error: status=0x%02x\n", status);
      
      if (priv->current_xfer)
        {
          priv->current_xfer->status = RA8E1_I2C_STATUS_ERROR;
...
int spi_gy912_dma_interrupt_handler(int irq, void *context, void *arg)
{
  ...
  if (priv->current_xfer)
    {
      priv->current_xfer->status = RA8E1_I2C_STATUS_COMPLETE;
