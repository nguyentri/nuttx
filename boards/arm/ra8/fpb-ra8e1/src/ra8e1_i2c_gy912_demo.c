...
static bool g_gy912_spi_initialized = false;
...
int gy912_i2c_initialize(void)
{
  int ret;

  if (g_gy912_spi_initialized)
    {
      return OK;
    }
...
int ra8e1_gy912_read_data(struct gy912_data_s *data)
{
  ...
  if (!g_gy912_spi_initialized || data == NULL)
    {
      return -EINVAL;
    }
