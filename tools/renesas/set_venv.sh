...
echo "[!] WARNING: curl SSL verification is disabled (CURL_OPTIONS=--insecure)"

# === Curl SSL workaround (optional, for development only) ===
# To disable SSL verification for curl, set CURL_NO_SSL_VERIFY=1 before running the script
if [ "${CURL_NO_SSL_VERIFY}" = "1" ]; then
  export CURL_OPTIONS="--insecure"
  echo "[!] WARNING: curl SSL verification is disabled (CURL_OPTIONS=--insecure)"
else
  export CURL_OPTIONS=""
